...
class OrderController extends Controller
{
  ...
  public function purchasehistory()
  {
    ...
    if (Auth::check()) {
      ...
      $bookings = DB::table('workshop_booking')
        ->select(['workshop_booking.id', 'workshop_booking.user_id', 'workshop_booking.workshop_id', 'workshop_booking.order_id', 'workshop_booking.transaction_id',
        'workshop_booking.payment_method', 'workshop_booking.total_amount', 'workshop_booking.currency', 'workshop_booking.currency_icon', 'workshop_booking.status',
         'workshop_booking.created_at', 'workshops.image as workshop_image', 'workshops.title as workshop_title',
         'workshops.start_date as start_date', 'workshops.start_time as start_time', 'workshops.end_date as end_date', 'workshops.end_time as end_time',])
        ->where('workshop_booking.user_id', Auth::user()->id)
        ->join('workshops', 'workshop_booking.workshop_id', '=', 'workshops.id')
        ->orderBy('workshop_booking.id', 'desc')
        ->get();

        // dd($bookings);

      return view('front.purchase_history.purchase', compact('orders', 'course', 'refunds', 'bookings'));
    }
  ...
  public function bookings()
  {
      $bookings = DB::table('workshop_booking')
        ...
        ->orderBy('workshop_booking.id', 'desc')
        ->get();
        return view('front.purchase_history.my-bookings', compact('bookings'));
  }
