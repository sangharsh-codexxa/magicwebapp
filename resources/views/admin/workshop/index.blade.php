...
<div class="col-md-4 col-lg-4">
  <div class="widgetbar">
    ...
    <button type="button" class="float-right btn btn-primary-rgba mr-2" data-toggle="modal" data-target="#myModal">
      <i class="feather icon-plus mr-2"></i>{{ __('Add Workshops') }}</a>
  </div>
</div>
            ...
            <table id="datatable-buttons" class="table table-striped table-bordered">
              ...
              <tbody id="sortable">
                ...
                <tr class="sortable" id="id-{{ $cat->id }}">
                  ...
                  <td>{{$cat->title}}</td>
                  <td>{{$cat->start_date . ' ' . $cat->start_time}}</td>
                  <td>{{$cat->end_date . ' ' . $cat->end_time}}</td>
                  <td>{{price_format($cat->fee)}}</td>
                  <td>
                              ...
                              <div class="row">
                                <div class="col-md-12">
                                  <div class="form-group">
                                    <label for="exampleInputTit1e">{{ __('Workshop') }}:<sup
                                  ...
                                  <br>
                                    ...
                                    <div class="input-group mb-3">
                                      ...
                                      @if(isset($cat['image']))
                                      <img src="{{ url('/images/workshop/'.$cat['image']) }}" class="image_size" />
                                      @endif 
                                    </div>
                                </div>
        ...
        <div class="modal-dialog modal-sm">
          <div class="modal-content">
            ...
            <div class="modal-footer">
              ...
              <form method="post" action="{{url('workshop/'.$cat->id)}}" data-parsley-validate class="form-horizontal form-label-left">
                {{ csrf_field() }}
                {{ method_field('DELETE') }}
...
<div class="modal fade" id="myModal" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="myModal">{{ __('Add Workshop') }}</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>

      </div>
      <div class="modal-body">
        <form id="demo-form2" method="post" action="{{url('workshop/store')}}" data-parsley-validate class="form-horizontal form-label-left" autocomplete="off" enctype="multipart/form-data">
          {{ csrf_field() }}

          <div class="row">
            <div class="col-md-12">
              <label for="c_name">{{ __('Title') }}:<sup class="redstar">*</sup></label>
              <input placeholder=" Please Enter Workshop name" type="text" class="form-control" name="title" required="">
            </div>
          </div>
          <div class="row">
            <div class="col-md-12">
              <label for="c_name">{{ __('Workshop fee') }}:<sup class="redstar">*</sup></label>
          ...
          <div class="row">
            <div class="col-md-12">
              <label for="c_name">{{ __('Description') }}:<sup class="redstar">*</sup></label>
          ...
          <div class="row">
            <div class="col-md-4">
              <label for="c_name">{{ __('Start Date') }}:<sup class="redstar">*</sup></label>
              <input placeholder=" Select Date" type="date" class="form-control" name="start_date" required="">
            </div>
            <div class="col-md-4">
              <label for="c_name">{{ __('Start Time') }}:<sup class="redstar">*</sup></label>
          ...
          <div class="row">
            <div class="col-md-4">
              <label for="c_name">{{ __('End Date') }}:<sup class="redstar">*</sup></label>
              <input placeholder=" Select Date" type="date" class="form-control" name="end_date" required="">
            </div>
            <div class="col-md-4">
              <label for="c_name">{{ __('End Time') }}:<sup class="redstar">*</sup></label>
          ...
          <div class="row">
            ...
            <div class="col-md-4">
              <label for="exampleInputDetails">{{ __('Status') }}:<sup class="redstar text-danger">*</sup></label><br>
              <input id="status_toggle" type="checkbox" class="custom_toggle" name="status" checked />
            </div>

            <div class="col-md-8">
              <label for="c_name">{{ __('Venue') }}:<sup class="redstar">*</sup></label>
